...
import type { MonitorEntryCSVRequestConfig } from "./types.ts";
 ...
 */
class MonitorEntryCSVRequestOptions {
  [key: string]: string;
   ...
   */
  constructor(options: MonitorEntryCSVRequestConfig) {
...
export function getMonitorEntriesCSVUrl(
  ...
  const params = new MonitorEntryCSVRequestOptions(config);
  return getApiUrl(
    `monitors/${config.monitorId}/entries/${config.field}/csv`,
    params,
  );
}
